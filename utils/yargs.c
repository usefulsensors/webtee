...
bool yargs_load_from_file(const YargsFlag* flags, int flags_length,
  ...
  const size_t bytes_read = fread(file_contents, 1, file_size, file);
  if (bytes_read != file_size) {
    fprintf(stderr, "Flags_LoadFromIniFile(): Expected to read %ld bytes from '%s', but only read %ld\n",
      file_size, filename, bytes_read);
    return false;
  }
